...
(function(definition, global) {
    ...
    prototype = modules["dom/element/emerge"].extend({
        ...
        _constructor: function(element, settings) {
            var self = prototype._parent._constructor.call(this, element, settings), defaultRatio, sources, i = 0, source, caption;
            defaultRatio = parseFloat(self.getAttribute("data-ratio") || 1);
            sources = self.find('[itemprop="source"],[itemprop="contentUrl"]');
            self._candidates = [];
            self._container = mDomElement.create("<div />").setStyles({
                position: "relative",
                display: "block",
                width: "100%",
                height: 0,
    ...
    mDomElement.create(global).on("resize orientationchange", delayOnResize);
    return prototype;
}, this);