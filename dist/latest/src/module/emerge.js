...
;(function(pDefinition, window) {
	...
	var definition = function definition() {
		return window.qoopido.shared.module.initialize('module/emerge', pDefinition, arguments);
	};

	if(typeof define === 'function' && define.amd) {
		define([ '../emitter', '../unique', '../element', '../function/merge' ], definition);
	} else {
		definition(window.qoopido.emitter, window.qoopido.unique, window.qoopido.element, window.qoopido.function.merge);
	}
}(function(mPrototype, mUnique, mElement, merge, namespace, window, document, undefined) {
	'use strict';
	...
	function checkState() {
		...
		if(self._element.isVisible() && (self._element.style.visibility !== 'hidden' || self._settings.visibility === false)) {
	...
	prototype = mPrototype.extend({
		...
		_constructor: function(element, settings) {
			...
			prototype._parent._constructor.apply(self, arguments);
			...
			if(intervals[settings.interval] === undefined) {
				...
				intervals[settings.interval] = window.setInterval(function() { tick(settings.interval); }, settings.interval);
			}

			self._viewport = {};
			self._uuid     = mUnique.uuid();
			self._element  = mElement.create(element);
			...
			window.on(LISTENER_RESIZE, function() { instanceOnResize.call(self); });
	...
	window.on(LISTENER_RESIZE, onResize);
	onResize();
